...
LIVE_CASES = [
    ...
    pytest.param(
        "get_capital_gains",
        lambda w: w.get_capital_gains("VTI", period="5y"),
        _assert_series_payload,
        marks=pytest.mark.xfail(reason="Yahoo frequently returns no capital gains data for this endpoint.", strict=False),
    ),
    ...
    ("get_industry_top_growth_companies", lambda w: w.get_industry_top_growth_companies("software-infrastructure"), _assert_dataframe_payload),
    ("get_industry_top_performing_companies", lambda w: w.get_industry_top_performing_companies("software-infrastructure"), _assert_dataframe_payload),
    ...
    ("get_earnings_dates", lambda w: w.get_earnings_dates("AAPL", limit=4, offset=0), _assert_dataframe_payload),
    ...
    pytest.param(
        "get_sustainability",
        lambda w: w.get_sustainability("AAPL"),
        _assert_dataframe_payload,
        marks=pytest.mark.xfail(reason="Yahoo fundamentals sustainability data is often unavailable.", strict=False),
    ),
    ...
    ("get_economic_events_calendar", lambda w: w.get_economic_events_calendar(start="2026-01-01", end="2026-03-31"), _assert_dataframe_payload),
